...
export type BubbleGraffitiLandingProps = {
  /** The big balloon word over the slab. Four to seven letters fill it best. */
  word?: string
  /** Lettered in black at the top, crowned. Clicking it makes the word hop. */
  brand?: string
  /** Mono line under the brand. */
  tagline?: string
  /** Pill links in the top bar. They fold into the menu on narrow screens. */
  nav?: LandingLink[]
  /** Extra entries that only live in the menu. */
  menu?: LandingLink[]
  /** Lines on the right, under the brand. */
  servicesTop?: string[]
  /** Lines on the lower left. */
  servicesBottom?: string[]
  /** First year. Shared leading digits are set once, big. */
  from?: string
  /** Last year. */
  to?: string
  /** The small word between the two years. */
  until?: string
  /** Mono credit under the years. */
  studio?: string
  searchPlaceholder?: string
  /** Play the bubble-graffiti preloader first, then wipe into the page. */
  intro?: boolean
  /** Length of that intro, in ms. */
  introDuration?: number
  palette?: BubblePalette
  /** Must be a definite length — never "100%". */
  height?: string
  minHeight?: string
  onNavigate?: (label: string, index: number) => void
  onSearch?: (query: string) => void
  /** Fires once the intro has cleared (or straight away with `intro={false}`). */
  onReady?: () => void
  className?: string
}

const NAV: LandingLink[] = [{ label: "Home" }, { label: "About me" }, { label: "Project" }]
const MENU: LandingLink[] = [{ label: "Playground" }, { label: "Contact" }]
const PYRAMID = ["........XX..........", ".....XXXXXXX........", "...XXXXXXXXXXXX.XX..", "XXXXXXXXXXXXXXXXXXXX"]

/** Digits roll up like a slot machine; everything else is set as is. */
function Roll({ text, at }: { text: string; at: number }) {
  return (
    <>
      {Array.from(text).map((c, i) =>
        /\d/.test(c) ? (
          <span key={i} className="bgl-roll">
            <span className="bgl-strip" style={cssVars({ "--n": Number(c) + 10, "--i": at + i })}>
              {Array.from({ length: 20 }, (_, d) => <span key={d}>{d % 10}</span>)}
            </span>
          </span>
        ) : (
          <span key={i}>{c}</span>
...
function Years({ from, to, until }: { from: string; to: string; until: string }) {
  let pre = 0
  while (pre < Math.min(from.length, to.length) - 2 && from[pre] === to[pre]) pre++
  const head = from.slice(0, pre)
  return (
    <div className="bgl-years" role="img" aria-label={from + " " + until + " " + to}>
      <div className="bgl-y" aria-hidden="true">
        {head ? <span className="bgl-y-big"><Roll text={head} at={0} /></span> : null}
        <span className="bgl-y-stack">
          <span className="bgl-y-top">
            <Roll text={from.slice(pre)} at={2} />
            <em>{until}</em>
          </span>
          <span className="bgl-y-bot">
            <Roll text={to.slice(pre)} at={4} />
          </span>
        </span>
      </div>
    </div>
...
function Landing({
  ...
  onNavigate,
  onSearch,
}: Required<Omit<BubbleGraffitiLandingProps, "intro" | "introDuration" | "palette" | "height" | "minHeight" | "onNavigate" | "onSearch" | "onReady" | "className">> &
  Pick<BubbleGraffitiLandingProps, "onNavigate" | "onSearch">) {
  ...
  React.useEffect(() => {
    ...
    const onDown = (e: PointerEvent) => {
      ...
      if (!menuRef.current?.contains(t) && !burgerRef.current?.contains(t)) setOpen(false)
    }
  ...
  const go = (i: number, l: LandingLink) => (e: React.MouseEvent) => {
    if (!l.href) e.preventDefault()
    setActive(i)
    setOpen(false)
    onNavigate?.(l.label, i)
  }

  // Parallax is written straight to CSS variables so moving the pointer never
  // re-renders the drawing.
  ...
  const onLeave = () => {
    bodyRef.current?.style.setProperty("--mx", "0")
    bodyRef.current?.style.setProperty("--my", "0")
  }

  const all = [...nav, ...menu]

  return (
    <div className="bgl-body" ref={bodyRef} onPointerMove={onMove} onPointerLeave={onLeave}>
      <div className="bgk-grid" />
      <div className="bgl-layer bgl-d2">
        <Squiggles />
      </div>
      <div className="bgl-layer bgl-d1">
        <div className="bgk-artbox">
          <HeroArt word={word} waveKey={waveKey} />
        </div>
      </div>
      <div className="bgl-layer bgl-d3">
        <div className="bgl-pyr" aria-hidden="true">
          <svg viewBox="0 0 440 88" preserveAspectRatio="xMidYMax meet">
            <PixelBlock rows={PYRAMID} x={0} y={0} cell={22} k0={0.2} />
          </svg>
        </div>
      </div>

      <div className="bgk-mark-wrap">
        <Wordmark text={brand} tagline={tagline} onPoke={() => setWaveKey((k) => k + 1)} />
      </div>

      <h1 className="bgl-sr">{word + " — " + brand}</h1>

      <ul className="bgl-svc bgl-svc-top">
        {servicesTop.map((s, i) => (
          <li key={i} style={cssVars({ "--i": i })}><span>{s}</span></li>
        ))}
      </ul>
      <ul className="bgl-svc bgl-svc-bot">
        {servicesBottom.map((s, i) => (
          <li key={i} style={cssVars({ "--i": i })}><span>{s}</span></li>
        ))}
      </ul>

      <div className="bgl-foot">
        <Years from={from} to={to} until={until} />
        <p className="bgl-studio">{studio}</p>
      </div>

      <nav className="bgl-nav" aria-label="Main">
        <ul className="bgl-links">
          {nav.map((l, i) => (
            <li key={i}>
              <a className="bgl-pill" href={l.href ?? "#"} aria-current={active === i ? "page" : undefined} onClick={go(i, l)}>
                {l.label}
              </a>
            </li>
          ))}
        </ul>
        <div className="bgl-tools">
          <form
      ...
      {open && (
        <div className="bgl-menu" id={menuId} ref={menuRef}>
          <ul>
            {all.map((l, i) => (
              <li key={i} style={cssVars({ "--i": i })}>
                <a href={l.href ?? "#"} onClick={go(i, l)} aria-current={active === i ? "page" : undefined}>
                  <small>{String(i + 1).padStart(2, "0")}</small>
                  <span>{l.label}</span>
                  <svg viewBox="0 0 5 5" aria-hidden="true">
                    <path d="M0 2h3v-2h1v1h1v3h-1v1h-1v-2h-3z" />
                  </svg>
                </a>
              </li>
            ))}
...
/**
 * A playful full-bleed landing page in bubble-graffiti: blue balloon lettering
 * over a black slab on a cream grid, inflatable blobs on the edges, pixel
 * blocks, a starburst, a crowned wordmark and a year range that rolls in.
 *
 * By default it opens with the bubble-graffiti preloader and pixel-wipes into
 * the page, which starts on the same drawing the preloader ends on. Letters
 * can be poked and dragged; the star and the wordmark set the word hopping.
 */
export default function BubbleGraffitiLanding({
  word = "hello",
  brand = "kedhar",
  tagline = "made with no asset",
  nav = NAV,
  menu = MENU,
  servicesTop = ["Photo Graph", "Video Edit"],
  servicesBottom = ["Graphic Design", "Illustration"],
  from = "2024",
  to = "2026",
  until = "until",
  studio = "/ playmode creative",
  ...
  onReady,
  className = "",
}: BubbleGraffitiLandingProps) {
  const [stage, setStage] = React.useState<"intro" | "reveal" | "live">(intro ? "intro" : "live")
  const readyRef = React.useRef(onReady)
  readyRef.current = onReady

  React.useEffect(() => {
    if (!intro) readyRef.current?.()
  }, [intro])

  return (
    <section
      className={"bgk-root bgl-root" + (className ? " " + className : "")}
      style={{ ...paletteVars(palette), height, minHeight }}
      aria-label={brand}
    >
      <style>{KIT_CSS}</style>
...
const LANDING_CSS = `
.bgl-body{position:absolute;inset:0;--bgk-d:1.9s;--mx:0;--my:0}
.bgl-layer{position:absolute;inset:0;pointer-events:none;transition:transform .6s cubic-bezier(.2,.7,.3,1)}
.bgl-d1{transform:translate3d(calc(var(--mx) * -12px),calc(var(--my) * -9px),0)}
.bgl-d2{transform:translate3d(calc(var(--mx) * 10px),calc(var(--my) * 6px),0)}
.bgl-d3{transform:translate3d(calc(var(--mx) * -5px),calc(var(--my) * -3px),0)}
.bgl-sr{position:absolute;width:1px;height:1px;margin:-1px;padding:0;overflow:hidden;clip:rect(0 0 0 0);white-space:nowrap;border:0}

.bgl-nav{position:absolute;top:4.5cqh;left:12cqw;right:12cqw;z-index:10;display:flex;align-items:center;justify-content:space-between;gap:12px;animation:bgl-down .8s cubic-bezier(.3,1.4,.5,1) backwards .15s}
@keyframes bgl-down{from{transform:translateY(-140%);opacity:0}}
.bgl-links{display:flex;gap:10px;list-style:none;margin:0;padding:0}
.bgl-pill{display:inline-flex;align-items:center;height:32px;padding:0 16px;border:1.5px solid var(--bgk-ink);border-radius:999px;background:var(--bgk-paper);color:var(--bgk-ink);font:700 11px/1 ui-sans-serif,system-ui,sans-serif;letter-spacing:.14em;text-transform:uppercase;text-decoration:none;white-space:nowrap;transition:transform .25s cubic-bezier(.3,1.8,.5,1),box-shadow .25s,background .2s,color .2s}
.bgl-pill:hover,.bgl-pill:focus-visible{transform:translate(-2px,-2px);box-shadow:3px 3px 0 var(--bgk-balloon),3px 3px 0 1.5px var(--bgk-ink);outline:none}
.bgl-pill[aria-current="page"]{background:var(--bgk-ink);color:var(--bgk-paper)}
.bgl-tools{display:flex;align-items:center;gap:8px}
.bgl-search{display:flex;align-items:center;gap:6px;height:32px;padding:0 12px;border:1.5px solid var(--bgk-ink);border-radius:999px;background:var(--bgk-paper);transition:box-shadow .25s,transform .25s cubic-bezier(.3,1.8,.5,1)}
.bgl-search:focus-within{transform:translate(-2px,-2px);box-shadow:3px 3px 0 var(--bgk-balloon),3px 3px 0 1.5px var(--bgk-ink)}
.bgl-search svg{width:15px;height:15px;flex:none;fill:none;stroke:var(--bgk-ink);stroke-width:1.8;stroke-linecap:round}
.bgl-search input{width:clamp(70px,11cqw,140px);border:0;outline:none;background:transparent;color:var(--bgk-ink);font:500 11px/1 ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.14em;padding:0;margin:0;transition:width .35s cubic-bezier(.3,1.3,.5,1);-webkit-appearance:none;appearance:none}
.bgl-search input::placeholder{color:var(--bgk-ink);opacity:.35}
.bgl-search input:focus{width:clamp(110px,17cqw,210px)}
.bgl-burger{position:relative;width:32px;height:32px;flex:none;border:1.5px solid var(--bgk-ink);border-radius:50%;background:var(--bgk-paper);cursor:pointer;padding:0;transition:transform .25s cubic-bezier(.3,1.8,.5,1),background .2s}
.bgl-burger:hover,.bgl-burger:focus-visible,.bgl-burger[aria-expanded="true"]{background:var(--bgk-ink);outline:none}
.bgl-burger:hover{transform:rotate(90deg)}
.bgl-burger span{position:absolute;left:9px;right:9px;height:1.5px;background:var(--bgk-ink);transition:transform .3s cubic-bezier(.3,1.6,.5,1),opacity .2s,background .2s}
.bgl-burger span:nth-child(1){top:10px}
.bgl-burger span:nth-child(2){top:14.25px}
.bgl-burger span:nth-child(3){top:18.5px}
.bgl-burger:hover span,.bgl-burger:focus-visible span,.bgl-burger[aria-expanded="true"] span{background:var(--bgk-paper)}
.bgl-burger[aria-expanded="true"] span:nth-child(1){transform:translateY(4.25px) rotate(45deg)}
.bgl-burger[aria-expanded="true"] span:nth-child(2){opacity:0}
.bgl-burger[aria-expanded="true"] span:nth-child(3){transform:translateY(-4.25px) rotate(-45deg)}

.bgl-menu{position:absolute;top:calc(4.5cqh + 44px);right:12cqw;z-index:12;width:min(300px,86cqw);padding:10px;background:var(--bgk-paper);border:2px solid var(--bgk-ink);border-radius:20px;box-shadow:6px 6px 0 var(--bgk-ink);transform-origin:calc(100% - 16px) 0;animation:bgl-menu .45s cubic-bezier(.3,1.5,.5,1)}
@keyframes bgl-menu{from{transform:scale(.4);opacity:0}}
.bgl-menu ul{list-style:none;margin:0;padding:0}
.bgl-menu li{animation:bgl-item .4s cubic-bezier(.3,1.5,.5,1) backwards;animation-delay:calc(var(--i) * 45ms + 80ms)}
@keyframes bgl-item{from{transform:translateX(24px);opacity:0}}
.bgl-menu a{display:flex;align-items:center;gap:12px;padding:10px 12px;border-radius:12px;color:var(--bgk-ink);text-decoration:none;font:300 clamp(18px,2.4cqh,24px)/1.1 ui-sans-serif,system-ui,sans-serif;transition:background .2s,color .2s,padding .25s}
.bgl-menu a small{font:600 10px/1 ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.1em;opacity:.6}
.bgl-menu a span{flex:1}
.bgl-menu a svg{width:14px;height:14px;fill:currentColor;opacity:0;transform:translateX(-6px);transition:opacity .2s,transform .25s}
.bgl-menu a:hover,.bgl-menu a:focus-visible{background:var(--bgk-balloon);padding-left:18px;outline:none}
.bgl-menu a:hover svg,.bgl-menu a:focus-visible svg{opacity:1;transform:none}
.bgl-menu a[aria-current="page"] span{text-decoration:underline;text-decoration-thickness:2px;text-underline-offset:4px}

.bgl-svc{position:absolute;z-index:5;list-style:none;margin:0;padding:0;font:300 clamp(16px,4cqh,34px)/1.12 ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif;letter-spacing:-.01em}
.bgl-svc li{animation:bgl-rise .7s cubic-bezier(.3,1.4,.5,1) backwards;animation-delay:calc(.45s + var(--i) * .1s)}
@keyframes bgl-rise{from{transform:translateY(60%);opacity:0}}
.bgl-svc span{cursor:default;padding:0 .08em;background:linear-gradient(var(--bgk-balloon),var(--bgk-balloon)) no-repeat 0 85% / 0% 40%;transition:background-size .35s cubic-bezier(.3,1.2,.5,1)}
.bgl-svc span:hover{background-size:100% 40%}
.bgl-svc-top{top:17cqh;right:12cqw;text-align:right}
.bgl-svc-bot{bottom:22cqh;left:12cqw}

.bgl-foot{position:absolute;right:12cqw;bottom:5cqh;z-index:5;display:flex;flex-direction:column;align-items:flex-end}
.bgl-y{display:flex;align-items:stretch;font-weight:900;letter-spacing:-.06em;line-height:.8;font-size:clamp(46px,14cqh,150px);font-variant-numeric:tabular-nums}
.bgl-y-big{display:flex}
.bgl-y-stack{display:flex;flex-direction:column;justify-content:space-between;font-size:.5em;margin-left:.06em}
.bgl-y-top{display:flex;align-items:flex-start;gap:.12em}
.bgl-y-top em{font-style:normal;font-weight:800;font-size:.34em;letter-spacing:0;line-height:1;margin-top:.1em}
.bgl-y-bot{display:flex}
.bgl-roll{display:inline-block;height:1em;line-height:1;overflow:hidden;vertical-align:top;margin:-.1em 0}
.bgl-strip{display:flex;flex-direction:column;line-height:1;transform:translateY(calc(var(--n) * -1em));animation:bgl-roll 1.5s cubic-bezier(.2,.9,.2,1) backwards;animation-delay:calc(.35s + var(--i) * .09s)}
@keyframes bgl-roll{from{transform:translateY(0)}}
.bgl-studio{margin:.9em 0 0;font:600 clamp(8px,1.3cqh,11px)/1 ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.34em;text-transform:uppercase}

.bgl-pyr{position:absolute;left:50%;bottom:-1px;width:min(44cqw,64cqh);transform:translateX(-50%)}
.bgl-pyr svg{display:block;width:100%;height:auto;aspect-ratio:440 / 88}

@container (max-width: 700px){
  .bgl-nav{left:5cqw;right:5cqw;top:3.5cqh}
  .bgl-links{display:none}
  .bgl-tools{margin-left:auto}
  .bgl-search input{width:0}
  .bgl-search input:focus{width:36cqw}
  .bgl-menu{right:5cqw;top:calc(3.5cqh + 44px)}
  .bgl-svc{font-size:clamp(15px,5.4cqw,24px)}
  .bgl-svc-top{top:22.5cqh;right:6cqw}
  .bgl-svc-bot{bottom:26cqh;left:6cqw}
  .bgl-foot{right:6cqw;bottom:6cqh}
  .bgl-y{font-size:clamp(40px,19cqw,110px)}
  .bgl-pyr{left:0;transform:none;width:52cqw}
  .bgl-d1,.bgl-d2,.bgl-d3{transform:none}
}

@media (prefers-reduced-motion: reduce){
  .bgl-root *,.bgl-root *::before,.bgl-root *::after{animation-duration:1ms !important;animation-delay:0ms !important;transition-duration:1ms !important}
  .bgl-d1,.bgl-d2,.bgl-d3{transform:none}
}
`
